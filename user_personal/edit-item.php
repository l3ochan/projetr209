...
// Vérifier si l'utilisateur est bien le propriétaire du véhicule à modifier
$token = $_SESSION['token'];
$query = "SELECT ownerID FROM items WHERE id = ? AND ownerID = (SELECT id FROM users WHERE token = ?)";
...
if ($result->num_rows != 1) {
    // Afficher un message d'erreur 403 (interdit) si l'utilisateur n'est pas le propriétaire de l'annonce
    http_response_code(403);
    echo "<h1>403 Forbidden</h1>";
    echo "<p>Accès interdit. Vous n'êtes pas autorisé à modifier cette annonce.</p>";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Traitement de la suppression des images
...
<head>
    ...
    <title>AutoIUT - Modifier annonce</title>

</head>
<body>
    <h1 class="page-title">Modifier l'annonce</h1>
    <div class="separator"></div>
    <form method="post" action="index.php?page=edit-item&id=<?php echo $itemID; ?>" enctype="multipart/form-data">
        ...
        ?>
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["delete_images"])) {
                ...
                header("Location: https://projetr209.nekocorp.fr/index?page=edit-item&id=" . $itemID);
                exit();
            }
            ?>
